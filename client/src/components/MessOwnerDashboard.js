...
function MessOwnerDashboard() {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/providers/${myMess._id}`, {
        method: 'PUT',
  ...
  const handleFileUpload = (e) => {
    ...
    // Validate File Size (Max 1MB for smooth Base64 storage)
  ...
  const handleCreateMess = async (e) => {
    ...
    const payload = {
      ...
      imageUrl: "", // No default image, owner must add their own
            ...
            <form onSubmit={handleUpdatePlans} style={{ maxWidth: "500px" }}>
              <div className="mb-4">
                ...
                <input 
                  type="number" 
                  style={formInputStyle} 
                  value={plansForm.trial} 
                  onChange={e => setPlansForm({...plansForm, trial: e.target.value})} 
                  required 
                />
              </div>
              <div className="mb-4">
                ...
                <input 
                  type="number" 
                  style={formInputStyle} 
                  value={plansForm.weekly} 
                  onChange={e => setPlansForm({...plansForm, weekly: e.target.value})} 
                  required 
                />
              </div>
              <div className="mb-4">
                ...
                <input 
                  type="number" 
                  style={formInputStyle} 
                  value={plansForm.monthly} 
                  onChange={e => setPlansForm({...plansForm, monthly: e.target.value})} 
                  required 
                />
              </div>
